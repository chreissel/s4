...
class Gwpy_loader(Dataset):
    def __init__(self, root, normalize=True):
        background = self.fill_data(root+'/background.npz', normalize=normalize, label=0)
        signal = self.fill_data(root+'bbh_for_challenge.npy', normalize=normalize, label=1)
        self.inputs = np.concatenate([background[0], signal[0]], axis=0)
        self.targets = np.concatenate([background[1], signal[1]], axis=0)
...
if args.dataset == 'ligo':
    ...
    fulldata = Gwpy_loader(root='./data/ligo/', normalize=True)
    generator = torch.Generator().manual_seed(42) 
    ...
    d_output = 2 # two classes: background vs. bbh
